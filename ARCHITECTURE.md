...
Five pages, all under `src/app`:

- **`/` (landing)** — marketing/overview page: hero with two CTAs (`/intake`, `/dashboard`), a static "how it works" explainer (`PipelineStepper` rendered with `activeIndex={5}`, i.e. permanently in its "all complete" state — no polling or interactivity, just a visual walkthrough of the five steps), a row of impact stats framed around what the system *does* (steps, single API call, 100% logged) rather than fabricated business/traction numbers, and a `CompanyMarquee` — a horizontally scrolling logo/wordmark ticker (Acme Co, Northwind Retail, etc. — generic placeholder company names, not real entities). No live data, no form — this page never talks to the database.
- **`/intake`** — `IntakeForm` (styled as "Website Contact Form," with one-click hot/cold example fillers) submits to the intake API. While the request is in flight, `PipelineStepper` animates through the five steps live; on success, `ResultPanel` shows the score, temperature, lifecycle status, the CRM record written, and the Slack message text. This is what used to live at `/` before the landing page was split out.
- **`/dashboard`** — `LeadsTable`, polling `GET /api/leads` every 5s via SWR (`refreshInterval`), with temperature filter tabs, a status filter (`Select`: all/qualified/disqualified), a search box (matches name/email/company/reasoning, case-insensitive substring), client-side pagination (10/page), and a click-through detail `Sheet` showing the raw message, full qualification, routing log, and a "Reprocess" button.
- **`/docs`** — a real documentation page: pipeline overview (reuses `PipelineStepper`), a getting-started walkthrough, and an integrations summary (AI scoring / CRM / notifications), each description accurate to the actual adapter fallback behavior in `lib/scoring.ts`/`crm.ts`/`notify.ts`.
- **`/docs/api`** — the API reference: all three endpoints with accurate request/response JSON built from the real Prisma field names and route handlers (not fabricated), using the shared `CodeBlock` component (`src/components/sift/code-block.tsx`, a plain `<pre>`/`<code>` block — no syntax-highlighting dependency).

`SiteHeader`'s `active` prop takes `"home" | "intake" | "dashboard" | "docs"` and is passed by each page — `"docs"` doesn't correspond to a nav link (Docs is linked from the footer only, not primary nav), so passing it just means neither `Intake` nor `Dashboard` gets highlighted, which is correct for `/docs`/`/docs/api`. `SiteFooter` (Resources/Company link columns + social-icon placeholders) has no props and is mounted identically at the bottom of all five pages, right after `<main>` inside the same `flex min-h-screen flex-col` wrapper — `main`'s `flex-1` pushes it to the bottom of the viewport on short pages via the standard sticky-footer flex pattern, no extra CSS needed. Resource links (`Documentation` → `/docs`, `API Docs` → `/docs/api`) are real routes rendered via `next/link`; Company links (`Privacy Policy`/`Security Policy`/`Terms and Conditions`) and the social icons are deliberate `href="#"` placeholders — there's no real destination for them yet. The `<footer>` itself is `bg-muted` (deliberately distinct from the page's `bg-background`, matching how most real sites visually separate the footer band from page content) — same token `CardFooter`/hover states/`CodeBlock` already use, so it's an existing "recessed surface" convention rather than a new color.

**Presentation-only relabeling**: the dashboard's Source column shows "Import" for leads with `source: "seed"` (`SOURCE_LABELS` map + `displaySource()` in `leads-table.tsx`) — this is a display-layer mapping only, the underlying stored value is still literally `"seed"` everywhere else (DB, `scripts/seed.ts`, `npm run db:clear`, this doc's own "seed data is tagged" note below all still refer to the real value). Don't confuse the two when grepping for `"seed"`. Any source *not* in `SOURCE_LABELS` (e.g. `"website"`, or any future integration source) falls back through `titleCase()` (`src/lib/types.ts`) rather than showing the raw lowercase value — add a new entry to `SOURCE_LABELS` only when a source needs a name that title-casing can't produce (like `seed` → `Import`).

**`LeadsTable` filtering/pagination is entirely client-side.** `GET /api/leads` is fetched once (unfiltered) via SWR; temperature/status/search/pagination are all plain array `.filter()`/`.slice()` in the component. This was already the existing pattern before status/search/pagination were added (the temperature tabs never sent a `?temperature=` query param, despite the API route accepting one) and was kept deliberately — at this app's scale (tens of leads, not thousands) server-side filtering would be unnecessary API surface for no real benefit. If this table ever needs to handle a genuinely large dataset, that's the point to move filtering server-side into `leadRepository.findMany`.

**Resetting pagination when a filter changes, without an Effect**: `LeadsTable` needs `page` to snap back to 1 whenever the temperature tab, status filter, or search text changes — otherwise you can land on an empty page. Rather than `useEffect(() => setPage(1), [filters])` (which trips this project's `react-hooks/set-state-in-effect` rule, see §11), it uses React's documented "adjust state during render" pattern: a `filterKey` string is derived each render, compared against a `lastFilterKey` state value, and if they differ, both `setLastFilterKey` and `setPage(1)` are called directly in the render body under that `if`. This is not an anti-pattern — React explicitly supports conditional `setState` calls during render for exactly this "reset on a changing key" case, and it avoids an Effect (and its lint warning) entirely. `currentPage` is additionally clamped to `Math.min(page, totalPages)` on every render as a pure computation (no `setState`) so the page never goes out of range even if the underlying lead count shrinks between polls without a filter changing.
...
**Design system**: shadcn/ui (`components.json`, `style: "base-nova"`, built on Base UI rather than Radix) with a custom theme layered on top in `src/app/globals.css` — Space Grotesk (headings, `font-heading`) + Inter (body/UI, `font-sans` — the default for all shadcn/ui components) + IBM Plex Mono (code/data, `font-mono`) loaded via `next/font/google`, and `--cold`/`--warm`/`--hot` semantic tokens (still amber/red, unrelated to the brand color) mapped into shadcn's CSS-variable token system so `bg-cold`, `text-hot`, etc. work as normal Tailwind utilities. `--accent` is a leftover from the original warm-editorial palette (before the obsidian pivot below) and is no longer used anywhere as a visible color — every component that referenced it (the closing CTA card on `/`, the Slack-message preview box in `result-panel.tsx`, and `ui/select.tsx`'s dropdown-item focus state) has been moved to a neutral token (`bg-primary`/`bg-muted`). The CSS variable itself is still defined in `globals.css` for now (removing shadcn theme tokens outright isn't worth the churn), it's just unreferenced.
...
**Accessibility — text contrast**: light-mode `--muted-foreground` was `#7C8698`, only 3.42:1 against `--background` — below WCAG AA's 4.5:1 minimum for normal text (dark mode's `#A6AEBF` was already fine at 8.13:1). Darkened to `#5C6779`, which clears 4.5:1+ against `--background`, `--card`, and `--muted` alike (the three surfaces it's typically read on) — this fixes every use of the token at once: nav links, card descriptions, table cells, timestamps, help text, placeholders. Separately, `ui/tabs.tsx`'s inactive-tab color (`text-foreground/60`, used by the Hot/Warm/Cold filter on `/dashboard`) composited to 4.21:1 in light mode — bumped to `/65` (≈4.9:1) to clear AA too. If you add more low-emphasis text, check contrast against whatever surface it actually sits on, not just against `--background` — a translucent/opacity-based color can pass on one surface and fail on another.

**Sticky header**: `SiteHeader` (`src/components/sift/site-header.tsx`) is `sticky top-0 z-10 bg-background` so it stays pinned while page content scrolls underneath — the explicit `bg-background` is required, not cosmetic: without it, scrolled content would show through the header once it stops being part of normal document flow.

**Shared badge components**: `TemperatureBadge` and `StatusBadge` (`src/components/sift/{temperature,status}-badge.tsx`) are the single source of styling for their respective fields — every place a lead's temperature or status is shown (`LeadsTable`'s table cells and detail `Sheet`, `ResultPanel`'s qualification card) renders through these two components rather than ad-hoc `<Badge>` usage, so a style change in one place changes it everywhere. Both are given a fixed `w-20`/`w-28` so badges align into a clean vertical column regardless of label length (`"HOT"` vs `"WARM"`, `"QUALIFIED"` vs `"DISQUALIFIED"`) — `Badge`'s base classes already include `justify-center`, so the fixed width alone centers the label. `StatusBadge` maps `DISQUALIFIED` to the `destructive` badge variant (existing `bg-destructive/10 text-destructive` token, already used by `Button`'s destructive variant) and every other status to `secondary` (`bg-secondary`) — both are Prisma-enum values, which (like `Temperature` elsewhere in this codebase) need an explicit `as LeadStatus`/`as Temperature` cast when passed in, since Prisma's generated enum type isn't structurally assignable to this app's own `@/lib/types` unions without one.
...
src/
  app/
    ...
    intake/page.tsx         the intake form ("/intake")
    dashboard/page.tsx      leads dashboard page
    docs/page.tsx           documentation ("/docs")
    docs/api/page.tsx       API reference ("/docs/api")
  ...
  components/
    ...
    sift/                   app-specific components:
                               site-header, site-footer, theme-toggle, intake-form,
                               pipeline-stepper, result-panel, leads-table, code-block,
                               temperature-badge, status-badge, company-marquee
    ui/                     shadcn/ui primitives (button, card, table, sheet, select, ...)
  lib/
    ...
    utils.ts                cn() helper (shadcn) + formatDate() ("DD MMM YYYY, HH:MM", local time)
...
> **Seed data is tagged, not hidden.** Every lead `npm run seed` creates has `source: "seed"`, visible in the dashboard's Source column (displayed as **"Import"** — see the presentation-only relabeling note in §6 — but still `"seed"` in the database) right alongside real submissions (e.g. `source: "website"` from the intake form). Nothing distinguishes them beyond that field — so if you've run `npm run seed` and see leads you don't recognize, that's why. To clear *all* leads regardless of source (not just seeded ones) and have the dashboard go back to empty, run `npm run db:clear` (`scripts/clear-leads.ts` — `prisma.lead.deleteMany()`, cascades to `Qualification`/`RoutingLog`); `npm run db:studio` also works for deleting rows selectively.
